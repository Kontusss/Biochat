"""
BioAgent Service — clean wrapper around the Biomni A1 agent.
...
from biomni.core.errors import (
    AgentError,
    AgentInitError,
    AgentTaskError,
    AgentTimeoutError,
...
from biomni.core.settings import BiochatSettings, biochat_settings
...
class BioAgentService:
    """Manages a Biomni A1 agent instance with lazy init and caching.
            ...
            if self._agent.use_tool_retriever:
                ...
                try:
                    ...
                    if selected:
                        ...
                        trace_lines.append(
                            f"✅ 已选择 {tool_count} 个工具, {data_count} 个数据集, {lib_count} 个库"
                        )
                except Exception as e:
                    trace_lines.append(f"⚠️ 工具检索跳过: {e}")
    ...
    def _parse_agent_output(
        ...
        for entry in log_entries:
            # Match patterns like "from biomni.tool.genomics import ..."
            matches = re.findall(r"from\s+biomni\.tool\.(\w+)\s+import\s+(\w+)", entry)
