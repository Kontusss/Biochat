...
``biomni/tool/tool_registry.py`` (linear list scans + eager DataFrame)
with name/id index maps and a lazily-built document frame.

The legacy path ``biomni.tool.tool_registry`` remains as a thin adapter.
"""
...
class ToolRegistry:
    ...
    Args:
        ...
        profile: ``"full"`` registers every module; ``"minimal"`` only the
                 modules listed in ``biomni/tool/profiles.py``.
    """
