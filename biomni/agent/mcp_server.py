...
``A1._generate_mcp_wrapper_from_biomni_schema()``.

Exposes internal Biomni tools as an MCP (Model Context Protocol) server
...
def build_biomni_mcp_server(agent: "A1", tool_modules: list[str] | None = None) -> Any:
    """Create and return a FastMCP server exposing Biomni internal tools.
...
def _create_schema_aware_wrapper(
    ...
    """Generate a function wrapper with a signature matching Biomni schema.
