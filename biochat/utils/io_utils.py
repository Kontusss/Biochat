...
def load_all_tool_descriptions(profile: str = "full") -> dict:
    """Import all ``biochat.tool.tool_description.*`` modules and
    return a ``{module_name: description_list}`` mapping.

    Args:
        profile: ``"full"`` loads every attributed Biochat tool module;
                 ``"minimal"`` loads only the modules listed in
                 ``biochat/tool/profiles.py`` (competition demo +
    ...
    for field in _TOOL_FIELDS:
        module_key = f"biochat.tool.{field}"
        if profile == "minimal" and module_key not in MINIMAL_TOOL_MODULES:
            continue
        module_name = f"biochat.tool.tool_description.{field}"
        try:
