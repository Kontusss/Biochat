Biochat API Reference (biomni)
===============================

Biochat is built on the Biomni engine. This reference documents the underlying ``biomni`` package API.

.. toctree::
   :maxdepth: 2
   :caption: API Contents:

   api/agent/a1
   api/agent/env_collection
   api/agent/function_generator
   api/agent/qa_llm
   api/agent/react
   api/biorxiv_scripts/extract_biorxiv_tasks
   api/biorxiv_scripts/generate_function
   api/biorxiv_scripts/process_all_subjects
   api/config
   api/model/retriever
   api/task/base_task
   api/task/hle
   api/task/lab_bench
   api/tool/biochemistry
   api/tool/bioengineering
   api/tool/biophysics
   api/tool/glycoengineering
   api/tool/bioimaging
   api/tool/cancer_biology
   api/tool/cell_biology
   api/tool/database
   api/tool/genetics
   api/tool/genomics
   api/tool/immunology
   api/tool/literature
   api/tool/microbiology
   api/tool/molecular_biology
   api/tool/pathology
   api/tool/pharmacology
   api/tool/physiology
   api/tool/support_tools
   api/tool/synthetic_biology
   api/tool/systems_biology
   api/tool/tool_registry
   api/utils
