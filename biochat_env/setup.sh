...
# BioAgentOS - Biochat Environment Setup Script
...
DEFAULT_TOOLS_DIR="$(pwd)/biochat_tools"
TOOLS_DIR=""

echo -e "${YELLOW}=== Biochat Environment Setup ===${NC}"
...
install_cli_tools() {
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "# Added by biochat setup" >> setup_path.sh
...
main() {
    ...
    # Setup completed
    echo -e "\n${GREEN}=== Biochat Environment Setup Completed! ===${NC}"
    ...
    PATH=$(echo $PATH | tr ':' '\n' | grep -v "biochat_tools/bin" | tr '\n' ':' | sed 's/:$//')
    export PATH="$(pwd)/biochat_tools/bin:$PATH"
}
