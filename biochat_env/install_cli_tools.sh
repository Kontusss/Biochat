...
cleanup_old_installations() {
    ...
    local old_afs_dir="/afs/cs.stanford.edu/u/$(whoami)/biochat_tools"
    ...
    local old_home_dir="$HOME/biochat_tools"
...
echo "# Added by biochat setup" >> "$TOOLS_DIR/setup_path.sh"
    ...
    if [ -n "$profile_file" ]; then
        ...
        if [ -w "$profile_file" ]; then
            ...
            if ! grep -q "export PATH=\"$TOOLS_DIR/bin:\$PATH\"" "$profile_file"; then
                # Remove any old biochat_tools paths first
                if grep -q "biochat_tools/bin" "$profile_file"; then
                    echo -e "${YELLOW}Removing old biochat_tools paths from $profile_file...${NC}"
                    sed -i '/biochat_tools\/bin/d' "$profile_file"
                fi

                echo "" >> "$profile_file"
                echo "# Added by biochat setup" >> "$profile_file"
